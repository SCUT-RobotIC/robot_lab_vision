...
def broken_bridge_terrain(difficulty: float, cfg) -> tuple[list[trimesh.Trimesh], np.ndarray]:
    ...
    y_coords = np.arange(length_pixels, dtype=np.float32) * cfg.horizontal_scale - 0.5 * cfg.size[1]

    x_cycle_length = cfg.x_flat_length + cfg.groove_width
    y_cycle_length = cfg.y_flat_length + cfg.groove_width
    x_phase = np.mod(x_coords + 0.5 * x_cycle_length, x_cycle_length)
    y_phase = np.mod(y_coords + 0.5 * y_cycle_length, y_cycle_length)
    x_groove = x_phase >= cfg.x_flat_length
    y_groove = y_phase >= cfg.y_flat_length

    groove_mask = x_groove[:, None] | y_groove[None, :]
